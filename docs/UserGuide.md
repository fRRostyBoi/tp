...
* The name field `n/` only permits ASCII alphabets, spaces, hyphens (i.e. `-`), periods (i.e. `.`) and commas `,`. Names must start with an alphabet. For the `Add` and `Edit` commands, extraneous whitespaces around or between words are trimmed.
